...
<template>
    <div>
        ...
        <div v-if="ready && queues.length == 0" class="flex flex-col items-center justify-center card-bg-secondary p-5 bottom-radius">
        ...
        <table v-if="ready && queues.length > 0" class="table table-hover mb-0">
            ...
            <tr v-for="queue in queues" :key="queue">
                <td>
                    <router-link class="no-underline" :to="{ name: 'metrics-preview', params: { type: 'queues', slug: queue }}">
                        {{ queue }}
                    </router-link>
                </td>
            </tr>
