    ...
    <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-3 min-h-[60px] border-b border-gray-100 dark:border-gray-700">
                ...
                <button class="inline-flex items-center gap-1.5 px-4 py-2 bg-violet-600 text-white rounded text-sm font-medium hover:bg-violet-700 transition-colors" v-on:click.prevent="retry(job.id)">
            ...
            <div class="p-5 bg-gray-50 dark:bg-gray-900" v-if="ready">
                ...
                <div class="flex mb-2">
                    <div class="w-1/6 text-gray-500 dark:text-gray-400 text-sm">Attempts</div>
                    <div class="flex-1 text-sm text-gray-900 dark:text-gray-100">{{job.payload.attempts}}</div>
                </div>
                <div class="flex mb-2">
                    <div class="w-1/6 text-gray-500 dark:text-gray-400 text-sm">Retries</div>
                    <div class="flex-1 text-sm text-gray-900 dark:text-gray-100">{{job.retried_by.length}}</div>
                </div>
                <div class="flex mb-2" v-if="job.payload.retry_of">
                    <div class="w-1/6 text-gray-500 dark:text-gray-400 text-sm">Retry of ID</div>
                    <div class="flex-1 text-sm">
                         <a :href="Horizon.basePath + '/failed/' + job.payload.retry_of" class="text-violet-600 dark:text-violet-400 hover:underline">
                            {{ job.payload.retry_of }}
                        </a>
                    </div>
                </div>
                <div class="flex mb-2">
                    <div class="w-1/6 text-gray-500 dark:text-gray-400 text-sm">Tags</div>
                    <div class="flex-1 text-sm text-gray-900 dark:text-gray-100">{{ job.payload.tags && job.payload.tags.length ? job.payload.tags.join(', ') : '' }}</div>
                </div>
                ...
                <div class="flex">
                    <div class="w-1/6 text-gray-500 dark:text-gray-400 text-sm">Failed</div>
                    <div class="flex-1 text-sm text-gray-900 dark:text-gray-100">{{readableTimestamp(job.failed_at)}}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden mt-4" v-if="ready">
            <div class="flex items-center justify-between px-5 py-3 min-h-[60px] border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-sm font-semibold m-0">Exception</h2>
            </div>
            <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden mt-4" v-if="ready">
            <div class="flex items-center justify-between px-5 py-3 min-h-[60px] border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-sm font-semibold m-0">Exception Context</h2>
            </div>
            <div class="p-5 bg-[#292d3e] text-white">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden mt-4" v-if="ready && job.retried_by.length">
            <div class="flex items-center justify-between px-5 py-3 min-h-[60px] border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-sm font-semibold m-0">Recent Retries</h2>
            </div>
            ...
            <table class="w-full mb-0">
                ...
                <tr>
                    ...
                    <th class="bg-gray-50 dark:bg-gray-900 text-xs font-semibold text-gray-600 dark:text-gray-400 px-5 py-2 text-left">ID</th>
                    <th class="bg-gray-50 dark:bg-gray-900 text-xs font-semibold text-gray-600 dark:text-gray-400 px-5 py-2 text-right">Retry Time</th>
                </tr>
                </thead>

                <tbody>
                <tr v-for="retry in job.retried_by" class="hover:bg-gray-50 dark:hover:bg-gray-700 border-t border-gray-200 dark:border-gray-700">
                    <td class="px-5 py-3 align-middle">
                        <svg v-if="retry.status == 'completed'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="inline-block fill-emerald-500" style="width: 1.5rem; height: 1.5rem;">
                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                        </svg>
                        <svg v-if="retry.status == 'reserved' || retry.status == 'pending'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="inline-block fill-amber-500" style="width: 1.5rem; height: 1.5rem;">
                        ...
                        <svg v-if="retry.status == 'failed'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="inline-block fill-red-500" style="width: 1.5rem; height: 1.5rem;">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0v-4.5A.75.75 0 0110 5zm0 10a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
                        </svg>
                        <span class="ml-2 text-sm text-gray-900 dark:text-gray-100">{{ upperFirst(retry.status) }}</span>
                    </td>

                    <td class="px-5 py-3 align-middle w-px whitespace-nowrap text-sm">
                        <a v-if="retry.status == 'failed'" :href="Horizon.basePath + '/failed/'+retry.id" class="text-violet-600 dark:text-violet-400 hover:underline">
                            {{ retry.id }}
                        </a>
                        <span v-else class="text-gray-900 dark:text-gray-100">{{ retry.id }}</span>
                    </td>
